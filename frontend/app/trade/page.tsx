...
export default function Trade() {
    ...
    const handlePreviousSet = () => {
        if (pageOffset > 0) {
            setPageOffset(pageOffset - maxPageNumbersToShow);
        }
    };

    const handleNextSet = () => {
        if (pageOffset + maxPageNumbersToShow < totalPages) {
            setPageOffset(pageOffset + maxPageNumbersToShow);
        }
    };
    ...
    return (
        ...
        <main>
            <div className="max-w-6xl mx-auto px-4 sm:px-6 relative">
                ...
                {showpage ? <div className="relative overflow-x-auto overflow-y-hidden my-[8rem] mt-[0rem]">
                    <div className="flex flex-column sm:flex-row flex-wrap space-y-4 sm:space-y-0 items-center justify-between pb-4">
                        <div>
                            <button
                                onClick={() => setDropdown({ ...dropdown, show: !dropdown.show })}
                            ...
                            {/* <!-- Dropdown menu --> */}
                            <div className={`${dropdown.show ? "absolute top-12 left-0" : "hidden"} m-0 z-10 w-48 divide-y rounded-lg shadow bg-gray-700 divide-gray-600`}>
                                <ul
                        ...
                        <div className="relative">
                            ...
                            <input type="text" id="table-search" className="block p-2 ps-10 text-sm border rounded-lg w-80 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-blue-500 focus:border-blue-500" placeholder="Search for items" />
                        </div>
                    </div>
                    ...
                    <nav className="flex items-center flex-column flex-wrap md:flex-row justify-between pt-4" aria-label="Table navigation">
                        <span className="text-sm font-normal text-gray-400 mb-4 md:mb-0 block w-full md:inline md:w-auto">Showing <span className="font-semibold text-white">{(currentPage - 1) * 10 + 1}-{currentPage * 10}</span> of <span className="font-semibold text-white">{transactions.length}</span></span>
                        <ul className="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8">
                            <li>
                                <button onClick={handlePreviousSet} disabled={pageOffset === 0} className="flex items-center justify-center px-3 h-8 ms-0 leading-tight border rounded-s-lg bg-gray-800 border-gray-700 text-gray-400 hover:bg-gray-700 hover:text-white">Previous</button>
                            </li>
                            {getVisiblePageNumbers().map((number) => (
                                <li key={number}>
                                    <button onClick={() => setCurrentPage(number)} className="flex items-center justify-center px-3 h-8 leading-tight border bg-gray-800 border-gray-700 text-gray-400 hover:bg-gray-700 hover:text-white">{number}</button>
                                </li>
                            ))}
                            <li>
                                <button onClick={handleNextSet} disabled={pageOffset + maxPageNumbersToShow >= totalPages} className="flex items-center justify-center px-3 h-8 leading-tight border rounded-e-lg bg-gray-800 border-gray-700 text-gray-400 hover:bg-gray-700 hover:text-white">Next</button>
                            </li>
                        </ul>
                    </nav>
                </div> : <div className="text-center text-gray-400 text-4xl">Please sign in to view your transactions.</div>}
            </div>
        </main>
